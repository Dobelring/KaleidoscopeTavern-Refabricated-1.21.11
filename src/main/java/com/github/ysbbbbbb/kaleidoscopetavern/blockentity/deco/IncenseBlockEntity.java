...
public class IncenseBlockEntity extends BaseBlockEntity {
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, IncenseBlockEntity incense) {
        ...
        List<LivingEntity> entities = level.getEntitiesOfClass(
                ...
                e -> e.getType().builtInRegistryHolder().is(EntityTypeTags.UNDEAD) && e.isAlive()
        );
        ...
        for (LivingEntity entity : entities) {
            ...
            // 僵尸村民血量低于5点时转化为普通村民
            if (entity instanceof ZombieVillager zombieVillager && zombieVillager.getHealth() <= 5.0f && !zombieVillager.isConverting()) {
                zombieVillager.setVillagerConversionTime(60);
            }
        }
    }
}
