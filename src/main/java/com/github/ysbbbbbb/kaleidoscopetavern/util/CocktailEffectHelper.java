...
public class CocktailEffectHelper {
     ...
     */
    public record CollectedData(List<DrinkEffectData.Entry> effects, List<TextColor> colors) {
    }
    ...
    public static CollectedData collectFromStorage(ItemStackHandler storage) {
        ...
        List<TextColor> colors = Lists.newArrayList();
        ...
        for (int i = 0; i < storage.getSlots(); i++) {
            ...
            // 普通原料：通过 Tag 获取颜色
            var color = ColorUtils.ITEM_COLOR_CACHE.apply(ingredient.getItem());
            if (color != null) {
                colors.add(color);
            }
