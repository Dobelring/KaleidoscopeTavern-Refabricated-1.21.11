...
public class ShakerBlockEntityRender implements BlockEntityRenderer<ShakerBlockEntity> {
    ...
    public void render(
        ...
        animate(shaker.putState, ageInTicks);
    ...
    protected void animate(AnimationState animationState, float ageInTicks) {
        animationState.updateTime(ageInTicks, 1.0f);
        animationState.ifStarted(state ->
                KeyframeAnimations.animate(this.model, ShakerAnimation.PUT, state.getAccumulatedTime(), 1.0F, ANIMATION_VECTOR_CACHE)
        );
    }
}
