...
public final class CommonModelLoading {
    ...
    public static void init() {
        ModelLoadingPlugin.register(pluginContext -> pluginContext.modifyModelAfterBake()
                .register(ModelModifier.WRAP_PHASE, (model, context) -> wrapModel(model, context.topLevelId())));
    }

    private static BakedModel wrapModel(BakedModel model, ModelResourceLocation modelId) {
        if (model == null || modelId == null) {
            return model;
        }

        ResourceLocation blockId = ResourceLocation.fromNamespaceAndPath(modelId.id().getNamespace(), modelId.id().getPath());
