    ...
    public void onIntegrationInitialize() {
        ItemView.addServerRecipeProvider(list -> {
            ServerRecipeManager.INSTANCE.getRecipesForType(ModRecipes.BARREL_RECIPE).forEach(recipe -> {
                ...
                list.add(new BarrelServerRecipe(ItemStackTemplate.fromNonEmptyStack(recipe.result().getDefaultInstance()), ingredients, recipe.carrier()));
            });
            ServerRecipeManager.INSTANCE.getRecipesForType(ModRecipes.PRESSING_TUB_RECIPE).forEach(recipe -> {
                list.add(new PressingTubServerRecipe(ItemStackTemplate.fromNonEmptyStack(recipe.getFluid().getBucket().getDefaultInstance()), recipe.input()));
            });
        });
