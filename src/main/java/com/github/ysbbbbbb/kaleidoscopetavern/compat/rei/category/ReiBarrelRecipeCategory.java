...
public class ReiBarrelRecipeCategory implements DisplayCategory<ReiBarrelRecipeCategory.BarrelRecipeDisplay> {
    ...
    public static void registerDisplays(DisplayRegistry registry) {
        ReiUtil.INSTANCE.barrelRecipes().forEach(holder -> {
            ...
            List<EntryIngredient> outputs = List.of(ReiUtil.ofItemStack(recipe.result().getDefaultInstance().copyWithCount(16)));
    ...
    public static class BarrelRecipeDisplay extends BasicDisplay {
        ...
        public BarrelRecipeDisplay(RecipeHolder<BarrelRecipe> holder) {
            this(holder.id().identifier(), ReiUtil.ofIngredients(holder.value().ingredients(), holder.value().fluid()), ReiUtil.ofItemStacks(holder.value().result().getDefaultInstance()), ReiUtil.ofIngredient(holder.value().carrier()), holder.value().unitTime());
        }
